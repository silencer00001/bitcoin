...
int CMPTransaction::step2_Alert(std::string *new_global_alert_message)
{
  ...
  // authorized, decode and make sure there are 4 tokens, then then replace global_alert_message
  std::string alertString((const char *) &pkt + 4);
  alertString.resize(std::min(alertString.length(), (size_t) SP_STRING_FIELD_LEN));
  ...
  std::string message(alertString);
